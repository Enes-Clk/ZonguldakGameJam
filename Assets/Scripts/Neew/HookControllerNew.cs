...
{[Header("Kanca Ayarları")]
    ...
    private bool isInWater = false; // Hook denizde mi?\n    private bool isStarting = false; // Hook başlangıç hareketi yapıyor mu?
    ...
    void HookStartMovement()

    {

        // Kanca suya ne kadar indikten sonra hareket etmeye başlasın?
...

        {
            ...
            float distanceToWater = Vector2.Distance(transform.position, rodTip.position);

            if (distanceToWater >= startDepth)

            {

                isInWater = true; // Kanca suya girdi

                Debug.Log("Kanca suya girdi, hareket başlıyor!");
...

    {
        ...
        // Kancayı görünür yap ve geminin 5 birim aşağısına ışınla

        transform.position = rodTip.position + Vector3.down * 5f;

        isInWater = false; // Başlangıçta denizde değil
